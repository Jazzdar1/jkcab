...
export default function AdminPanel() {
  ...
  const handleUnlock = async (e: React.FormEvent) => {
    ...
    if (pinInput === masterPass) {
      // setIsUnlocked(true) and setIsAdmin(true) are enough for session
      setIsUnlocked(true);
      setIsAdmin(true);
  ...
  const handleSyncInitial = async () => {
    toast.promise(async () => {
      ...
      for (const item of VEHICLES) {
        const itemRef = doc(db, 'fleet', item.id);
        const itemSnap = await getDoc(itemRef);
        if (!itemSnap.exists()) {
          await setDoc(itemRef, { ...item, updatedAt: serverTimestamp() });
        }
      }

      // 3. Sync Drivers
      for (const item of DRIVERS) {
        const itemRef = doc(db, 'drivers', item.id);
        const itemSnap = await getDoc(itemRef);
        if (!itemSnap.exists()) {
          await setDoc(itemRef, { ...item, updatedAt: serverTimestamp() });
        }
      ...
      // 4. Sync Tour Packages
      for (const item of TOUR_PACKAGES) {
        const itemRef = doc(db, 'packages', item.id);
        const itemSnap = await getDoc(itemRef);
        if (!itemSnap.exists()) {
          await setDoc(itemRef, { ...item, updatedAt: serverTimestamp() });
        }
      ...
      // 5. Sync Rates
      for (const item of ROUTE_RATES) {
        // Generating a unique ID for rates based on route name if not present
        const rateId = item.route.replace(/[^a-zA-Z0-9]/g, '-').toLowerCase();
        const itemRef = doc(db, 'rates', rateId);
        const itemSnap = await getDoc(itemRef);
        if (!itemSnap.exists()) {
          await setDoc(itemRef, { ...item, updatedAt: serverTimestamp() });
        }
      ...
      success: 'All earlier data restored and synced!',
  ...
  return (
    <div className="flex h-screen bg-[#030b17] overflow-hidden font-sans text-white">
      <Toaster position="top-center" richColors />
      
      {/* Sidebar - Desktop */}
      <aside className="hidden md:flex flex-col w-72 bg-[#101c2b] border-r border-white/5 shadow-2xl z-50">
        ...
        <div className="mt-auto p-8 space-y-3">
          <Button 
            ...
            className="w-full justify-start space-x-3 h-14 rounded-2xl text-[10px] uppercase font-black tracking-widest border-dashed border-red-500/10 text-red-500 hover:bg-red-500/5 hover:border-red-500/30"
          >
          ...
          <Button 
            ...
            className="w-full justify-start space-x-3 h-14 rounded-2xl text-[10px] uppercase font-black tracking-widest border-dashed border-white/10 hover:bg-yellow-400/5 hover:border-yellow-400/30"
          >
          ...
          <Button 
            variant="ghost" 
            onClick={() => navigate('/')}
            className="w-full justify-start space-x-3 h-14 rounded-2xl text-[10px] uppercase font-black tracking-widest text-red-500 hover:bg-red-500/10"
          >
      ...
      {/* Main Content Area */}
      <main className="flex-1 flex flex-col min-w-0 bg-[#030b17] relative">
        
        {/* Top Navigation Bar */}
        <header className="h-20 bg-[#101c2b]/80 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-8 z-40">
          <div className="flex items-center flex-1 max-w-md bg-white/5 rounded-2xl px-4 h-12 group focus-within:ring-2 focus-within:ring-yellow-400/50 transition-all">
            <Search className="h-4 w-4 text-gray-400 group-focus-within:text-yellow-500" />
            <Input 
              placeholder="Instant Search..." 
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="border-0 bg-transparent focus-visible:ring-0 text-xs font-bold text-white placeholder:text-gray-600" 
            />
          </div>
          ...
          <div className="flex items-center space-x-4 pl-8">
            <div className="hidden sm:flex flex-col items-end mr-4 text-right">
              ...
              <span className="text-[8px] font-bold text-gray-600 tracking-widest leading-none">SECURE ACCESS GRANTED</span>
            </div>
          ...
          <div className="p-8 md:p-12 max-w-7xl mx-auto w-full">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
              <div>
                ...
                <h2 className="text-6xl font-black text-white tracking-tighter uppercase leading-none font-display">
              ...
              {activeTab === 'settings' ? (
                ...
                <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
                  ...
                  <DialogContent key={editingItem?.id || 'new-item'} className="max-w-2xl bg-white dark:bg-[#111] border-0 rounded-[3rem] shadow-2xl p-10 overflow-y-auto max-h-[90vh]">
            ...
            {activeTab === 'settings' && (
              <Card key={`settings-${siteSettings?.updatedAt?.seconds || 'initial'}`} className="rounded-[3rem] border-0 bg-white dark:bg-[#111] shadow-2xl p-10 mt-8 mb-20">
                <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {/* Branding Group */}
